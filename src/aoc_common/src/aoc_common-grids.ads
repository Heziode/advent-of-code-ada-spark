...
package AoC_Common.Grids is
   ...
   type Grid_Type (Rows : Row_Range; Cols : Col_Range) is private;
   ...
   with
      Post => Get_Rows (Create'Result) = Rows and then
              Get_Cols (Create'Result) = Cols;
   ...
   with
      Pre => Row <= G.Rows and then Col <= G.Cols;
   ...
   with
      Pre => Pos.Row >= 1 and then Pos.Row <= G.Rows and then
             Pos.Col >= 1 and then Pos.Col <= G.Cols;
   ...
   function Is_Valid_Position
      (G   : Grid_Type;
       Pos : Coordinate) return Boolean
   is (Pos.Row >= 1 and then Pos.Row <= G.Rows and then
       Pos.Col >= 1 and then Pos.Col <= G.Cols)
   with Inline;

private

   type Grid_Array is array (Row_Range range <>, Col_Range range <>) of Element_Type;

   type Grid_Type (Rows : Row_Range; Cols : Col_Range) is record
