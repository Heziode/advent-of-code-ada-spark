...
package body AoC_Common.Directions is
   ...
   Cardinal_Offsets :
     constant array (Cardinal_Direction) of Direction_Offset :=
       [North => (Delta_Row => -1, Delta_Col => 0),
        East  => (Delta_Row => 0, Delta_Col => 1),
        South => (Delta_Row => 1, Delta_Col => 0),
        West  => (Delta_Row => 0, Delta_Col => -1)];
   ...
   function Rotate_Clockwise
     (Dir : Cardinal_Direction) return Cardinal_Direction is
   begin
      case Dir is
         when North =>
            return East;
   ...
   begin
      return
        (Row => Pos.Row + Offset.Delta_Row, Col => Pos.Col + Offset.Delta_Col);
   end Move;

   function Move (Pos : Coordinate; Dir : Direction) return Coordinate is
