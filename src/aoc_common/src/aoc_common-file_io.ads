...
is
   ...
   function Find_First (Source : String; Pattern : Character) return Natural
   with
     Post =>
       Find_First'Result = 0
       or else (Find_First'Result in Source'Range
                and then Source (Find_First'Result) = Pattern);
   ...
   function Find_Last (Source : String; Pattern : Character) return Natural
   with
     Post =>
       Find_Last'Result = 0
       or else (Find_Last'Result in Source'Range
                and then Source (Find_Last'Result) = Pattern);
   ...
   with Pre => Is_Digit (C), Post => Digit_Value'Result in 0 .. 9;
   ...
   with
     Post =>
       (if Parse_Natural'Result.Valid then Parse_Natural'Result.Value >= 0);
   ...
   with
     Pre => S'Length > 0,
     Post =>
       First_Non_Whitespace'Result = 0
       or else First_Non_Whitespace'Result in S'Range;
   ...
   with
     Pre => S'Length > 0,
     Post =>
       Last_Non_Whitespace'Result = 0
       or else Last_Non_Whitespace'Result in S'Range;
   ...
   generic
      ...
      with procedure Process_Line (Line : String; Data : in out Data_Type);
   procedure For_Each_Line
     (Filename : String; Success : out Boolean; Data : in out Data_Type)with
     SPARK_Mode => On;
