...
The Elves do have the manual for the machines, but the section detailing the initialization procedure was eaten by a [Shiba Inu](https://en.wikipedia.org/wiki/Shiba%5FInu). All that remains of the manual are some indicator light diagrams, button wiring schematics, and [joltage](3) requirements for each machine.
...
**Algorithm:** GF(2) Gaussian Elimination

Each button press toggles lights (XOR operation). Since pressing twice cancels out, each button is pressed 0 or 1 times. This reduces to solving `Ax = t (mod 2)` where:
- `A` is the matrix of button masks (columns)
- `t` is the target light state
- `x` is the solution vector (which buttons to press)

We find the minimum Hamming weight solution via:
1. Gaussian elimination over GF(2) with column pivoting
2. Enumerate free variable combinations (2^k for k free vars)
3. Back-substitute to find full solution
4. Track minimum weight across all solutions

**Expected Results:**
- Example: Part 1 = 7, Part 2 = 33
- Part 1: 415
- Part 2: 16663
...
# Run
./bin/main_aoc_2025_day_10

# SPARK Proof
alr exec -- gnatprove -P aoc_2025_day_10.gpr --mode=silver -j0
```
